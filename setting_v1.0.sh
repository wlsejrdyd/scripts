...
참고사항 )
root 접근권한은 막혀있습니다.
발급받으신 계정으로 관리자 권한 획득바랍니다.
????의 보안정책으로, 서버방화벽을 이용하시는 것을 권장 드립니다.
정책을 추가하실 경우 firewall-cmd 명령어를 이용바랍니다.
패스워드는 반드시 접속 후 변경하여 별도 관리 해주시길 바랍니다."
}
...
case ${num1} in 1 )
	...
	nmcli con mod ${dev1} ipv4.addresses ${ip1}/${prefix1} ipv4.gateway ${gw1}
	...
	echo -e "PING_CHECK : $(ping -c 1 ${gw1} | grep ${gw1} | grep from)"
	exit 0
;;
